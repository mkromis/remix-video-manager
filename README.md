...
- user sessions, and verifying them [./app/session.server.ts](app/services/session.server.ts)
...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create remix-video-manager-9b0a
  fly apps create remix-video-manager-9b0a-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-video-manager-9b0a
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-video-manager-9b0a-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'remix-video-manager-9b0a-staging' does not match app name in config file 'remix-video-manager-9b0a'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name remix-video-manager-9b0a-db
  fly postgres attach --app remix-video-manager-9b0a remix-video-manager-9b0a-db

  fly postgres create --name remix-video-manager-9b0a-staging-db
  fly postgres attach --app remix-video-manager-9b0a-staging remix-video-manager-9b0a-staging-db
  ```
